...
function psqlParamInsert(payloadJson: string): void {
  ...
  const sql = `
    INSERT INTO mojo_scores (company_id, computed_at, total_score, component_scores, explanation, methodology_version)
    SELECT (j->>'company_id')::uuid, (j->>'computed_at')::timestamptz, (j->>'total_score')::numeric,
           j->'component_scores', j->'explanation', j->>'methodology_version'
...
const companies =
  psqlJson<CompanyRow[]>(
    `SELECT coalesce(json_agg(json_build_object('id', id, 'name', name) ORDER BY created_at), '[]'::json) FROM companies WHERE id <> '${CB1_FROZEN_ID}';`,
  ) ?? [];
...
for (const company of companies) {
    ...
    psqlJson<SignalRow[]>(
      `SELECT coalesce(json_agg(json_build_object('id', id, 'source_type', source_type, 'event_date', event_date, 'confidence_to_use', confidence_to_use)), '[]'::json)
       ...
       WHERE company_id='${company.id}' AND signal_band='outside'
         ...
         AND length(trim(coalesce(evidence_excerpt,''))) > 10;`,
    ) ?? [];
  ...
  const signals: OutsideSignalInput[] = signalsRaw.map((s) => ({
    id: s.id,
