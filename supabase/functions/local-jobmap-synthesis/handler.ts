...
async function callLocalSynthesis(args: {
  ...
  const odiContextBlock = renderOdiGrounding(args.journeyDefinitions, {
    desiredOutcome: odiCtx?.desired_outcome,
    outcomeLeadingIndicator: odiCtx?.outcome_leading_indicator,
    recurringChallenge: odiCtx?.recurring_progress_challenge,
  });
...
export type SynthesisDeps = { createClient: typeof createClient };

/** A journey key that is not the customer spine or the internal set is a market run: it must be
 *  scoped (selected_maps_only) and strict (require_model) — Gate 2a flags, enforced (2026-09-15). */
export function requiresScopedRunFlags(journeyKey: string) {
  return !isCustomerJourneyKey(journeyKey) && journeyKey !== "internal";
}

export async function handleLocalJobmapSynthesis(
  req: Request,
  deps: SynthesisDeps = { createClient },
): Promise<Response> {
  ...
  try {
    ...
    // Market runs (any key that is not customer/internal) must be scoped and strict.
    // Before 2026-09-15 nothing enforced this — a caller could regenerate a market
    // set unscoped and with the deterministic fallback allowed.
    const marketKeys = requestedMaps.map((map) => map.journey_key).filter(requiresScopedRunFlags);
    if (marketKeys.length > 0 && (!selectedMapsOnly || !requireModel)) {
      return json({
        ok: false,
        error: "scoped_run_flags_required",
        journey_keys: marketKeys,
        message: `Journey ${marketKeys.map((k) => `'${k}'`).join(", ")} is a market set: the run must send selected_maps_only=true and require_model=true — nothing was written.`,
      }, 400);
    }

    const supabase = deps.createClient(supabaseUrl, serviceRole);
    const authHeader = req.headers.get("Authorization");
    const anonClient = deps.createClient(
      supabaseUrl,
      anonKey,
    ...
    // R2 (2026-09-15): every journey in the run resolves its OWN market definition by
    // (company_id, journey_key), live only, no latest-row fallback — and all of them
    // resolve BEFORE any read of the corpus or any write. One missing key refuses the
    // whole run. The customer spine and the internal set take this same path.
    const runJourneyKeys = [
      ...requestedMaps.map((map) => map.journey_key),
      // An unscoped run always writes the customer spine (parser injection + the
      // post-normalization unshift below), so its definition is part of the run.
      ...(selectedMapsOnly ? [] : ["customer"]),
    ];
    const resolved = await resolveJourneyDefinitions(
      supabase as unknown as { from: (t: string) => any },
      companyId,
      runJourneyKeys,
    );
    if (!resolved.ok) {
      console.log(`[local-jobmap-synthesis] refused: ${resolved.message}`);
      return json({ ok: false, error: resolved.error, missing_journey_keys: resolved.missing, message: resolved.message }, 422);
    }
    const definitionsByKey = resolved.byKey;
    ...
    const evidenceContext = {
      ...
      // R2: one definition per requested journey_key — the model grounds each set on
      // its own executor/job, never on "the company's" latest definition.
      journey_definitions: journeyDefinitions.map((d) => ({
        journey_key: d.journey_key, job_executor: d.job_executor, chooser: d.chooser, jtbd: d.jtbd,
      })),
    ...
    const industryExclusions = buildCompanyVocabExclusions([
      ...
      ...journeyDefinitions.flatMap((d) => [d.job_executor, d.jtbd, d.chooser]),
    ]);
    ...
    // R1 (2026-09-15): the model's market_definition output is NOT consumed. This
    // generator never writes odi_market_definitions — minting or updating a definition
    // is generate-market-hypothesis's job only. The definitions this run used were
    // resolved by key above and are left exactly as they were.
    ...
    return json({
      ...
      summary: {
        ...
        // R1: this generator never touches the definition rows it read.
        market_definition: "untouched",
      },
      artifacts: {
