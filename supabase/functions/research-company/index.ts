  ...
  try {
    ...
    const jobStepGroundingRules = thinEvidenceColdStart
      ? `Evidence is thin for this company (no uploaded internal evidence). For CUSTOMER journeys, anchor each of the ${JTBD_CHECKPOINT_COUNT} checkpoints to the ${anchorSourceNote} hypotheses below — adapt the wording to this company's job executor and category, but keep the job-progression intent of each checkpoint:\n` +
        `${anchorBlock}\n` +
        `- Customer journey steps: set designed=true, has_gap=false, gap_note="", evidence_status=implied, evidence_confidence at most 50, and evidence_basis="A starting map inferred from your public presence — not yet validated with your customers." (do NOT name any framework or methodology in evidence_basis)\n` +
    ...
    // ones, and PRESERVES findings it didn't re-confirm (absence != contradiction).
    // Curated ('manual') and declared ('internal_declared') rows are never loaded.
    ...
    // odi_needs delete removed — reconciled below (PCT-2).
