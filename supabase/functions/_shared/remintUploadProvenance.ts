...
export async function applyRemint(supabase: Sb, companyId: string, plans: RemintProposalPlan[], opts: { actor?: string; note?: string }): Promise<Array<{ proposal_id: string; superseded: number; minted: number; struck: number; ledger_id: string | null }>> {
  ...
  for (const plan of plans) {
    ...
    const { data: p } = await supabase.from("file_proposals").select("id, company_id, file_name, source_type, summary, evidence, contradictions, framework_results, questions_to_verify, suggested_areas, confidence, confidence_reason").eq("id", plan.proposal_id).maybeSingle();
