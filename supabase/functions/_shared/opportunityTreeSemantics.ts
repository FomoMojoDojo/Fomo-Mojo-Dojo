import {
  ...
  type DesiredOutcomeParts,
  type OutcomeLevel,
  STAGE_PERMITTED_LEVELS,
} from "./desiredOutcome.ts";
...
/** Rejects outcomes that describe internal states rather than observable behavior */
...
/** Actor words that signal a post-sale / delivery context */
const POST_SALE_ACTOR_PATTERN = /\b(client|clients|existing client|engagement|engagements)\b/i;

/** Actor words that signal a pre-sale / conviction context */
...
export function validateDesiredOutcome(candidate: DesiredOutcomeCandidate): ValidationResult {
  ...
  // Surface behavior-first warnings so AI pipelines can detect gaps.
  ...
  // Alignment signals — surfaced for AI pipelines and repair logic, but do not
  // block `valid`. Prompts control alignment; validators check structure.
  ...
  if (candidate.stage && candidate.level) {
    const permitted = STAGE_PERMITTED_LEVELS[candidate.stage] ?? [];
    if (permitted.length > 0 && !permitted.includes(candidate.level as OutcomeLevel)) {
