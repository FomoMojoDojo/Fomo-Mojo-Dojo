...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Dimension 4: buyer/executor alignment (role lexicon, 70b band) ──────────
    {
      ...
      // industry, not its buyers; showing them to the judge and asking about buyers is
      // a category error (first IAQM calibration run did exactly that).
      ...
      const internalRoles = rolesIn(`${internalExecutor}\n${internalChooser}\n${internalJtbd}`);
      ...
      const internalSide = `${internalExecutor}\n${internalChooser}`;
      if (internalRoles.size === 0 || externalRoles.size === 0) {
        dimensions.push({
          dimension: "buyer_executor_alignment", verdict: "insufficient_evidence", method: "deterministic", score: null, identity: null,
          internal_cited: internalSide.slice(0, 200),
          external_cited: externalRoleTexts.slice(0, 4).map((m) => ({ source: m.source, text: m.text.slice(0, 160) })),
          detail: internalRoles.size === 0
            ? "Internal definition names no recognizable buyer/executor roles — internal side silent on this dimension."
            : "Public evidence names no recognizable buyer/executor roles — the market has no independent buyer read yet.",
        });
      } else {
        const score = containment(internalRoles, externalRoles);
        // Identity follows the judged evidence: the role-bearing sample, not the blob.
        const identity = await comparisonIdentity("buyer_executor_alignment", internalSide, roleSample);
        // Same calibration law. The prompt names executor and chooser separately — the
        // executor (who experiences the job) and the chooser (who buys) are different
        // roles internally, and public evidence usually speaks in buyer language.
        let verdict: DimensionFinding["verdict"]; let method: DimensionFinding["method"] = "deterministic"; let detail: string;
        if (score >= 0.5) { verdict = "aligned"; detail = `Internal roles (${Array.from(internalRoles).join(", ")}) substantially present in public evidence roles (${Array.from(externalRoles).join(", ")}); containment ${score.toFixed(3)}.`; }
        else {
          const band = await bandVerdict(identity,
            `Answer with exactly one word: YES or NO.\nIs the market described by PUBLIC EVIDENCE buying/executing the SAME job as the internal definition below? Consider that the job EXECUTOR (who experiences the job) and the CHOOSER (who buys) may be described differently by outside sources.\n\nINTERNAL JOB EXECUTOR: ${internalExecutor.slice(0, 300)}\nINTERNAL CHOOSER (buyer): ${internalChooser.slice(0, 300)}\n\nPUBLIC EVIDENCE ROLES: ${Array.from(externalRoles).join(", ")}\nPUBLIC EVIDENCE (role-bearing items):\n${roleSample}\n\nAnswer:`, score);
          method = band.method;
          ...
          else { verdict = band.aligned ? "aligned" : "divergent"; detail = `Semantic band (role containment ${score.toFixed(3)}): ${band.aligned ? "same" : "different"} buyers per ${band.method === "stored" ? "stored verdict" : "llama3:70b"}.`; }
        }
        dimensions.push({ dimension: "buyer_executor_alignment", verdict, method, score: Number(score.toFixed(4)), identity, internal_cited: internalSide.slice(0, 200), external_cited: externalRoleTexts.slice(0, 4).map((m) => ({ source: m.source, text: m.text.slice(0, 160) })), detail });
      }
    }
    ...
    const priorVerdicts = ((priorBasis?.latest as { dimensions?: DimensionFinding[] } | undefined)?.dimensions ?? [])
      .map((d) => `${d.dimension}=${d.verdict}`).sort().join("|");
    const currentVerdicts = dimensions.map((d) => `${d.dimension}=${d.verdict}`).sort().join("|");
    const changed = !isBaseline && priorVerdicts !== currentVerdicts;
    // Alert law: baseline quiet; unchanged quiet; changed comparison WITH divergence ⇒ alert.
    const driftState = isBaseline ? "aligned" : changed && divergent.length > 0 ? computedState : "aligned";
