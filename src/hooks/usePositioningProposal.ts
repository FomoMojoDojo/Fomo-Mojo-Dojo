...
export type PositioningProposalRow = {
  id: string;
...
export function usePositioningProposal(companyId?: string, refreshKey = 0) {
  const [loading, setLoading] = useState(false);
  const [proposal, setProposal] = useState<PositioningProposalRow | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (fetchError) {
        ...
        setProposal(data as PositioningProposalRow | null);
      }
