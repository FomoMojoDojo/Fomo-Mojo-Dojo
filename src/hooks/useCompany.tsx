...
export interface Company {
  ...
  name: string;
  website: string | null;
  /** The company declared it has no public site (2026-09-16): nothing is crawled, searched for, or born from its name. */
  no_public_site?: boolean | null;
  created_by: string;
  created_at: string;
  quarter?: string | null;
...
export function CompanyProvider({ children }: { children: ReactNode }) {
  ...
  const fetchCompanies = useCallback(async (signal?: AbortSignal) => {
    ...
    const baseSelect =
      "id,name,website,created_by,created_at,frozen,mojo_score,potential_score,projected_score,evidence_status,evidence_note,last_scored_at,area_scores_json,industry_key,no_public_site";
