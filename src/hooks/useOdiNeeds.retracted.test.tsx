...
describe("useOdiNeeds — keyed read (gate 3)", () => {
  ...
  it("the no-key path keeps select('*') and ALSO excludes status=retracted (gate 3 delta, ruling 1)", async () => {
    calls.needs.length = 0;
    ...
    expect(q.filters).toEqual(["eq company_id=co", "neq status=retracted"]);
  });
});
