...
import {
  type ProposalAcceptPayload, type FoundationArea, FOUNDATION_AREAS, FOUNDATION_AREA_TO_AREA_KEY, AREA_KEY_TO_FOUNDATION,
  applyAreaTags, type SourceRow, isFileBacked, areaDisplayLabel, fileProposalProcessingBadgeStyle, fileProposalProcessingBadgeText,
  fileProposalReviewCountsText, isProposalStale, isQueuedPlaceholderSummary, proposalPriority, inferSuggestedAreasFromProposal,
  proposalProgressPercent, MONO, LABEL_TINY, DeleteConfirmPanel, ProposalReviewPanel, WORKSHOP_TAGS, type WorkshopTag, type ProcessingStatus,
} from "../inputsShared";
...
export default function InputsTab({
  ...
  async function handleDeleteFile(row: SourceRow, mode: "file-only" | "file-and-unlink") {
    ...
    try {
      ...
      if (mode === "file-and-unlink" && row.filePath) {
        await unlinkNeedsFromFilePath(row.filePath);
      }
      await refetchFiles();
    } catch {
      await refetchFiles();
  ...
  async function handleAcceptProposal(row: SourceRow, proposal: FileProposalRow, payload: ProposalAcceptPayload) {
    ...
    await acceptFileProposal({ fileId: row.id, rawTags: row.rawTags, proposalId: proposal.id, areas: payload.areas });
  ...
  async function handleRejectProposal(proposal: FileProposalRow) {
    ...
    setProposalPanelId(null);
    await rejectFileProposal(proposal.id);
    await refetchProposals();
  }
