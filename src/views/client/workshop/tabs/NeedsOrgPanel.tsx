...
import { OpportunityProposalSection, SuggestEditLane } from "../opportunitiesShared";
...
export default function NeedsOrgPanel({
  ...
  // Human edit lane: which declared need is being authored (the lane itself — draft + submit state —
  // MOVED to opportunitiesShared.SuggestEditLane, Opportunities Tier 1 lift 2026-09-12).
                ...
                <div style={{ display: "grid", gridTemplateColumns: "60px 1fr auto", alignItems: "start", gap: "0 16px", borderBottom: pendingProposal ? "none" : `1px solid ${D.hairlineFaint}`, padding: "16px 0 16px", opacity: hierarchyOffStrategy ? 0.58 : undefined }}>
                  ...
                  <div>
                    ...
                    {onAuthorProposal && need.provenance_type === "internal_declared" && !pendingProposal && (
                      <SuggestEditLane
                        need={need}
                        open={authoringNeedId === need.id}
                        canSuggest={canSuggest}
                        onOpen={() => setAuthoringNeedId(need.id)}
                        onClose={() => setAuthoringNeedId(null)}
                        onAuthorProposal={onAuthorProposal}
                      />
                    )}
                    {onCheckSurfaceDrift && (
                      <button
