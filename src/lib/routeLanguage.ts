...
function buildTemplate(input: RouteLanguageInput): RewriteTemplate | null {
  ...
  if (/grant application|application requirements/.test(text)) {
    return {
      title: "Reduce responder delay caused by unclear grant requirements",
      shortDescription: "Make grant expectations easier to act on so departments can request support without unnecessary back-and-forth.",
      whyHint: "This route matters if unclear requirements are slowing down access to support that already exists.",
    };
  }

  if (/data-driven allocation|allocation insights|future funding priorities/.test(text)) {
    return {
      title: "Make future funding decisions easier to defend",
      shortDescription: "Use visible impact patterns to shape future allocation choices before donor confidence weakens.",
      whyHint: "This route matters if future funding priorities are still hard to justify with visible evidence.",
    };
  }

  if (/usage and impact|impact tracking|monitoring systems|funded equipment/.test(text)) {
    return {
      title: "Make funded impact visible before donor confidence drifts",
      shortDescription: "Show how funded support is being used so donor confidence can rest on visible impact, not assumptions.",
      whyHint: "This route matters if support quality is real but hard to see once funds are deployed.",
    };
  }

  if (/fund distribution|allocation records|distribution transparency/.test(text)) {
    return {
      title: "Protect donor trust with clearer fund allocation proof",
      shortDescription: "Make allocation choices easier to see so donors and agencies can trust how support is being distributed.",
      whyHint: "This route matters if trust weakens when funding decisions are hard to trace after the fact.",
    };
  }

  if (/funding review|review and approve|county chiefs|support delivery/.test(text)) {
    return {
      title: "Shorten the gap between responder need and funding response",
      shortDescription: "Reduce the lag between urgent need and visible action so support feels responsive, not bureaucratic.",
      whyHint: "This route matters if responder needs move faster than the current decision cycle.",
    };
  }

  if (/first responder engagement|resource utilization|mental wellness programs|active participation/.test(text)) {
    return {
      title: "Reduce drop-off between funded support and responder use",
      shortDescription: "Close the gap between funded programs and actual first-responder participation so support does not stall after approval.",
      whyHint: "This route matters if the support is funded but not yet reaching enough of the people it was meant to help.",
    };
  }

  if (/rapid needs communication|urgent equipment|urgent.*needs/.test(text)) {
    return {
      title: "Shorten the gap between urgent need and visible response",
      shortDescription: "Give agencies a faster way to surface urgent needs so time-sensitive support is not lost in the queue.",
      whyHint: "This route matters if the need becomes urgent before the organization can even see it clearly.",
    };
  }

  if (/feedback frequency|changing support needs|agencies/.test(text)) {
    return {
      title: "Keep support priorities aligned with changing responder needs",
      shortDescription: "Make changing field needs visible soon enough to keep support decisions relevant.",
      whyHint: "This route matters if support decisions are aging faster than the field conditions they are meant to address.",
    };
  }

  if (/funding cycle reporting|donors and stakeholders|long-term impact/.test(text) || /governance|community participation|donor willingness/.test(hypothesis)) {
    return {
  ...
  if (/mental wellness impact metrics|effectiveness of mental health support/.test(text)) {
    return {
      title: "Make mental wellness impact visible enough to sustain support",
      shortDescription: "Show whether mental wellness funding changes outcomes in ways that can hold donor confidence over time.",
      whyHint: "This route matters if mental wellness support is valued but still hard to defend with visible outcome proof.",
    };
  }
