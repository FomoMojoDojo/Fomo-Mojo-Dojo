...
describe("fill plan — step order + resume", () => {
  it("runs the six steps in dependency order", () => {
  ...
  it("stepsFrom() with no arg returns the full order; --from resumes; unknown throws", () => {
    ...
    expect(stepsFrom("deltas_public")).toEqual(["deltas_public", "open_questions", "status_conflict", "score"]);
    expect(stepsFrom("score")).toEqual(["score"]);
