...
- Object path format: `<user_id>/<input_id>/<timestamp>-<filename>`
...
Use one local folder per company and place files under the input key they belong to:
...
You can add subfolders under each input key.
...
- Default is dry run.
- Files not inside an input-key path are skipped.
...
Use this to mirror them back into `Client_Files/<Company>/<input_key>/...`:
